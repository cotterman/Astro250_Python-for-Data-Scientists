...
def get_spreadsheet_info():
    ...
    mydf = pd.DataFrame(index=range(10000), 
        columns=['profile_ID','profile_link', 'date_posted', 'date_funded', 'funding_time',
                'patient_age', 'location', 'treat_cost'])
    patient_num = 0
    for line in open(file):
        ...
        for row in rows: 
            ...
            mydf.location[patient_num] = ages_and_locs[0][1]
...
def get_facial_expression(profile_count):
    ...
    for line_num, line in enumerate(open(file)):
        ...
        df_smile.smile_scale[line_num] = line.split(",")[0] #smile score 0 - 1

    print "\nWe have smile info for " , line_num, " profiles."
...
def get_text_and_photo_data(mydf_main, download):
    ...
    for row_num in range(mydf_main.shape[0]):
...
def write_data_csv(mydf):
    ...
    days_diff_list = []
    for row_num in range(mydf.shape[0]):
        ...
        if pd.notnull(mydf.funding_time[row_num]):
            ...
            days_diff = None
        if days_diff<1:
            #print "warning: negative funding time for row " , row_num
            days_diff = None
